...
class GPTBiLSTMHierarchyExperiment2:
    def __init__(
        self,
        dataset_name,
        train_x,
    ...
    def build_model_classifier(self, run_number=0):
        ...
        if output_embedding_layer_nomalized:
            ...
            seq_embedding = TimeDistributed(Bidirectional(LSTM(192)))(seq_embedding)
        else:
            seq_embedding = TimeDistributed(Bidirectional(LSTM(output_dim)))(
                output_gpt_embedding
            )
        ...
        if output_embedding_layer_nomalized:
            self.classifier_model = Model(
                ...
                name="GPT_Hierachy_BiLSTM_NORM_Classifier",
            )
        else:
            self.classifier_model = Model(
                inputs=input_model,
                outputs=output_layer,
                name="GPT_Hierachy_BiLSTM_Classifier",
            )
