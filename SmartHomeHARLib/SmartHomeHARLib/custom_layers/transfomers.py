...
class TokenAndPositionEmbedding2(layers.Layer):
    def __init__(self, maxlen, vocab_size, embed_dim, mask_zero=False, **kwargs):
        ...
        self.day_emb = keras.layers.Embedding(
            input_dim=8,
            output_dim=self.embed_dim,
            mask_zero=mask_zero)

        self.hour_emb = keras.layers.Embedding(
            input_dim=25,
            output_dim=self.embed_dim,
            mask_zero=mask_zero)

        self.minutes_emb = keras.layers.Embedding(
            input_dim=61,
            output_dim=self.embed_dim,
            mask_zero=mask_zero)
        ...
        self.supports_masking = self.token_emb.supports_masking
        ...
        config.update(
            {
                ...
                "maxlen": self.maxlen,
                "embed_dim": self.embed_dim,
                "mask_zero": self.token_emb.mask_zero,
            },
        )
        return config
    ...
    def call(self, x1, x2, x3, x4):
        ...
        embedded_tokens = self.token_emb(x1)

        maxlen = tf.shape(x1)[1]
        positions = tf.range(start=0, limit=maxlen, delta=1)
        positions = self.pos_emb(positions)

        day_positions = self.day_emb(x2)
        hour_positions = self.hour_emb(x3)
        minutes_positions = self.minutes_emb(x4)

        outputs = positions + embedded_tokens + \
            day_positions + hour_positions + minutes_positions

        # outputs = embedded_tokens + day_positions + hour_positions + minutes_positions
...
class TokenAndPositionEmbeddingHour(layers.Layer):
    def __init__(self, maxlen, vocab_size, embed_dim, mask_zero=False, **kwargs):
        super().__init__(**kwargs)
    ...
    def call(self, x1, x2):
        ...
        positions = tf.range(start=0, limit=maxlen, delta=1)
        positions = self.pos_emb(positions)
        hour_positions = self.hour_emb(x2)

        outputs = positions + embedded_tokens + hour_positions
...
class TransformerBlock(layers.Layer):
    def __init__(self, embed_dim, num_heads, ff_dim, rate=0.1, activation="gelu", **kwargs):
        ...
        self.ffn = keras.Sequential(
            [layers.Dense(ff_dim, activation=self.activation),
...
class GPT_Block(layers.Layer):
    def __init__(self, embed_dim, num_heads, rate=0.1, use_causal_mask=False,  **kwargs):
        ...
        self.mlp = keras.Sequential(
            [layers.Dense(4*embed_dim, activation='gelu'),
             layers.Dense(embed_dim), layers.Dropout(rate)]
        )
    ...
    def call(self, inputs, mask=None):
        ...
        attention_output = self.att(
            lnout1, lnout1, use_causal_mask=self.use_causal_mask, attention_mask=self.mask)
