...
class ELMoEventEmbedder(BaseEmbedding):
    ...
    def tokenize(self, filter="", lower=False, vocabulary_dict=None, oov_token="<UNK>"):
        ...
        for i, sentence in enumerate(self.sentences):
            ...
            self.sentences[i] = "<sep> " + sentence + " <sep>"
        ...
        self.sentences = np.array(
            self.tokenizer.texts_to_sequences(self.sentences), dtype=object
        )
    ...
    def prepare(self):
        ...
        for i in range(len(f_inputs)):
            ...
            bi, bo = sliding_window(
                np.array(b_inputs_revesed),
                np.array(f_inputs_reversed),
                self.window_size,
                self.step,
            )
    ...
    def prepare_3(self):
        ...
        self.forward_inputs, self.forward_outputs = progressive_sliding_window(
            self.sentences, self.sentences, self.window_size, self.step
        )

        # reverse for backward
        sentences_revesed = self.sentences[::-1]

        self.backward_inputs, self.backward_outputs = progressive_sliding_window(
            sentences_revesed, sentences_revesed, self.window_size, self.step
        )
            ...
            for sentence in self.sentences:
                for i in range(self.step, len(sentence) - 1, self.step):
                    ...
                    if i + self.step + self.window_size > len(sentence):
                        ...
                        b_inputs = sentence[
                            i + self.step : i + self.step + self.window_size
                        ]
    ...
    def __model(self):
        ...
        # forward
        forward_l1 = LSTM(
            self.embedding_size, return_sequences=True, name="forward_lstm_layer_1"
        )(forward_inputs_embedded)
        ...
        forward_l2 = LSTM(
            self.embedding_size, return_sequences=True, name="forward_lstm_layer_2"
        )(forward_l1)

        # backward
        backward_l1 = LSTM(
            self.embedding_size, return_sequences=True, name="backward_lstm_layer_1"
        ...
        backward_l2 = LSTM(
            self.embedding_size, return_sequences=True, name="backward_lstm_layer_2"
        )(backward_l1)
        ...
        model = Model(
            inputs=[forward_inputs, backward_inputs],
    ...
    def __model_3(self):
        ...
        if self.residual:
            forward_l1 = Add(name="forward_residual")(
                [forward_l1, forward_inputs_embedded]
            )

        # forward_l1 = BatchNormalization()(forward_l1)
        forward_l2 = LSTM(
            self.embedding_size, return_sequences=False, name="forward_lstm_layer_2"
        )(forward_l1)
        ...
        if self.residual:
            backward_l1 = Add(name="backward_residual")(
                [backward_l1, backward_inputs_embedded]
            )

        # backward_l1 = BatchNormalization()(backward_l1)
        backward_l2 = LSTM(
            self.embedding_size, return_sequences=False, name="backward_lstm_layer_2"
        )(backward_l1)
    ...
    def __model_4(self):
        ...
        model = Model(
            inputs=[forward_inputs, backward_inputs], outputs=output, name="ELMoLike"
        )
    ...
    def compile(self, embedding_weights=[], picture_path=None):
        ...
        perp = keras_nlp.metrics.Perplexity(from_logits=False, name="perplexity")
    ...
    def train(self, best_model_path=None, patience=20, log_dir=None, csv_path=None):
        ...
        # FOR MODEL 6
        self.model.fit(
            [self.forward_inputs, self.backward_inputs],
    ...
    def __elmo_model(self, model):
        ...
        forward_lstm_layer_1_weight = model.get_layer(
            "forward_lstm_layer_1"
        ).get_weights()
        backward_lstm_layer_1_weight = model.get_layer(
            "backward_lstm_layer_1"
        ).get_weights()
        forward_lstm_layer_2_weight = model.get_layer(
            "forward_lstm_layer_2"
        ).get_weights()
        backward_lstm_layer_2_weight = model.get_layer(
            "backward_lstm_layer_2"
        ).get_weights()
        ...
        self.elmo_model = tf.keras.models.Model(
    ...
    def get_elmo_embedding_vectors(self):
        ...
        with tqdm(
            total=len(vocabulary_index), desc="Get ELMo embedding vectors"
        ) as pbar:
            ...
            self.vectors = dict(
                zip(list(self.vocabulary.keys()), np.array(self.vectors))
            )

    def get_elmo_sentence_embedding(
        self, sentence, use_multiprocessing=True, workers=32, batch_size=2048
    ):
        """
    ...
    def get_elmo_embedding_layer(
        self, embedding_type="concat", trainable=False, mask_zero=True
    ):
        vocab_size = len(self.vocabulary)
        emb_dim = self.embedding_size
        ...
        forward_lstm_layer_1_weight = self.model.get_layer(
            "forward_lstm_layer_1"
        ).get_weights()
        backward_lstm_layer_1_weight = self.model.get_layer(
            "backward_lstm_layer_1"
        ).get_weights()
        forward_lstm_layer_2_weight = self.model.get_layer(
            "forward_lstm_layer_2"
        ).get_weights()
        backward_lstm_layer_2_weight = self.model.get_layer(
            "backward_lstm_layer_2"
        ).get_weights()
        ...
        elmo_embedding = tf.keras.models.Model(
            inputs=sentence_inputs, outputs=output, name="ELMO_embedding"
        )
